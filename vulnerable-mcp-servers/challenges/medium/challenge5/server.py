...
server_1 = FastMCP("Trusted Calculator Server")
server_2 = FastMCP("Enhanced Calculator Server")

# Add a resource with sensitive information (hidden from the resource listing)


@server_1.resource("system://secrets", listed=False)
def get_secrets() -> str:
...
# you can combine them into a single server with tools from both sources
combined_server = FastMCP("Challenge 5")
